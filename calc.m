...
n=param.n;
Sy=param.Sy;
D =[param.rho, param.rho*param.R, -2*param.sigma, 2, param.rho*9.8*param.R, cos(param.theta_zero/180*3.141592), -param.X2*param.R/param.sigma/param.alpha, -param.alpha, -param.R/(1/param.n+1)/(param.k^(1/param.n)), -param.X1*param.k/param.sigma/(param.R^(param.n-1)),-param.X1*param.R/param.sigma];
...
y0  = [0.00001; 0.00001; 3.141592/180*60;100]; % 89 deg is the initial contact angle to prevent singularity
yp0 = [0.000001 ; 0; 0; 10];
...
% %SCA  ------------------------------------------------
% [~,H_SCA] = ode45(@(t,h) ((c4-c2*h)/(c1*h))^s,T,0.001*0.0000000001);

% output=struct('SCA',H_SCA,'DCA',result_H_2step,'early',H_HD,'late',H_HD_later,'N',N);
